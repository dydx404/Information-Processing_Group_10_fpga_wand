...
| --- | --- | --- | --- | --- | --- |
| Yi | Early project setup and planning, wand-side track ownership at project start, and later system integration / delivery leadership | Primary implemented owner of major parts of the final presented system, including the adopted fallback FPGA centroid design path, PS/runtime code, UDP sender path, WandBrain UDP ingest/parse/runtime path, final integrated backend wrapper, database integration, live web console, and final end-to-end integration | Early exploratory work also exists on the branch history, including pre-final ESP32/IMU proof-of-concept work from the earlier architecture stage | `develop` branch history (treated as Yi’s delivery branch for report-preparation purposes), current adopted file structure under `FPGA/` and `software/`, and the final demonstrated system record | Confirmed |
| Wells | FPGA track ownership, including PL architecture, PS bindings, and general Zynq-board development | Active contribution to the final pre-demonstration integration, testing, debugging, and rapid development phase of the adopted system | A more ambitious FPGA image-processing design path, including custom HDL and hardware export artefacts, which did not become the final adopted centroid-based PL implementation | Wells branch commits (`6533bcc`, `19c422f`), project chronology, and final integration record | Confirmed |
...
| Mingze | FPGA node and local processing track, including node-side processing, local visual output, and associated hardware-side support | Final-system-relevant hardware and validation support, including camera hardware notes, camera-mount CAD assets, LED-wand CAD/schematic artefacts, auxiliary PYNQ-side test scripts, and active contribution to the final pre-demonstration integration/testing push | Current GitHub evidence does not by itself show primary ownership of the final adopted PS/runtime path or the final centroid-based PL implementation | `feat/hardware` branch commit `6dcf9b2` by `MSB_233`, added files under `hardware/camera/`, `hardware/wand/current_led_wand/`, and `FPGA/runtime/test/`, plus the final integration record | Confirmed |
...
| Physical sensing setup support: camera selection notes, camera-mount CAD, LED-wand CAD/schematic assets, and auxiliary node-side bring-up tests | Mingze | Yi and Wells contributed to final integration/testing around the same physical setup as applicable | `feat/hardware` branch commit `6dcf9b2`, artefacts under `hardware/camera/`, `hardware/wand/current_led_wand/`, and `FPGA/runtime/test/` | Safe wording is that Mingze contributed final-system-relevant hardware documentation, CAD assets, and auxiliary verification scripts supporting the adopted physical setup, while primary ownership of the final runtime remained elsewhere |
