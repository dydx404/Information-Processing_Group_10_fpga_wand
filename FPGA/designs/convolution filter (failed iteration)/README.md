...
- **Convolution filter:** Used for cleaning up the high frequency (salt/pepper) noise from the surrounding. Using a Gaussian image filter, the high-frequency noise will be filtered out.
...
#### Verilog files
The custom IP for the convolution filter is include in the folder verilog files. The hierarchical order is: <br>
<img width="582" height="482" alt="image" src="https://github.com/user-attachments/assets/297c6bb4-73bd-4c39-9317-56c87e007394" />
(All verilog files can be found in the folder except output buffer, which is a in-built IP) <br> 
The architecture is as described in the following: <br>
Refer to Vipin Kizheppatt course, the idea is to have, with a N x N size kernel, having (N+1) line buffer to load the image row pixels, alternatively switch between load mode and store mode, for which, when N line buffers are operating (outputing pixel in that row) the other one loads a new row of pixel from DDR (via DMA). And multiplexers to input different combination of rows into the kernel, then output to a output buffer for the next stage. <br>
