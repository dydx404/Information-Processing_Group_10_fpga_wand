...
Status: Integration handbook for `software/cloud/backend/versions/brain_v2_scoring`
...
Base runtime folder:
- `software/cloud/backend/versions/brain_v2_scoring`

Canonical repo-level cloud entrypoint:
- `software/cloud/main.py`
...
Practical note:
- For the current full app, operators usually launch `software/cloud/start_script.sh` from
  the repo root rather than running this package directly.

TX simulation scripts (repo root `software/tools/`):
- Long noisy stroke: `software/tools/wb_tx_long_noisy_stroke.py`
- Rate sweep sender: `software/tools/wb_tx_rate_test.py`
...
```bash
cd ~/fpga_wand/Information-Processing_Group_10_fpga_wand/software/cloud
...
```bash
python3 software/tools/wb_tx_long_noisy_stroke.py \
...
```bash
software/cloud/backend/versions/brain_v2_scoring/tools/run_observable_v2_overlay_demo.sh \
