...
usage() {
  cat <<EOF
Usage: software/cloud/backend/versions/brain_v2_scoring/tools/run_observable_v2_overlay_demo.sh [options]
...
TX_SCRIPT="${SOFTWARE_ROOT}/tools/wb_tx_long_noisy_stroke.py"
...
if ! curl -fsS "http://${API_HOST}:${API_PORT}/api/v2/templates" >/dev/null; then
  ...
  echo "Start the repo-level cloud app from software/cloud/start_script.sh and retry." >&2
  exit 1
fi
